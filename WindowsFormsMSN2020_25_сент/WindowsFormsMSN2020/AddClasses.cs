...
{
    public class MSNCalculation
    {
        public int GroupCount = 26;
        public MacroParams ActiveCore;
        public MacroParams Reflector;

        public double Temperature;
        public bool isSelfScreening;
        public bool isInfinityReflector;
        public double ReflectorTicknessRatio;
        public double HeatOutput;

        public List<CalcGeneration> CalcGenList;

        public MSNCalculation(double T, bool ss, bool ir, double r, double ho)
        {
            Temperature = T;
            isSelfScreening = ss;
            isInfinityReflector = ir;
            ReflectorTicknessRatio = r;
            HeatOutput = ho;
            ActiveCore = new MacroParams(this);
            Reflector = new MacroParams(this);
            CalcGenList = new List<CalcGeneration>();
        }

        public void Init(MacroIsotopePropertiesList ac, MacroIsotopePropertiesList r)
        {
            ActiveCore.Init(ac);
            Reflector.Init(r);
        }
    ...
    public class CalcGeneration
    {
        public int num;
        private MSNCalculation msnCalc;
        public OneGroupRParams ogpAC;
        public OneGroupRParams ogpR;
        public double[] IntegralFlow;
        public double[] NeutronValue;
        public CalcGeneration(MSNCalculation msn, int n)
        {
            msnCalc = msn;
            num = n;
            ogpAC = new OneGroupRParams();
            ogpR = new OneGroupRParams();
        }
    }

    public class OneGroupRParams
    {
    }

    public class MacroIsotopePropertiesList : List<MacroIsotopeProperties>
    {
        internal double InitTermalProperties(double T)
        {
            //вернем температуру нейтронного газа для данного состава среды
            double SIGMA_a = 0;
            double KSI_SIGMA_s = 0;
            //плотности в расчете уже поправлены с учетом температуры среды
            foreach (MacroIsotopeProperties Mip in this)
            {
                SIGMA_a += Math.Sqrt(293 / T) * (Mip.el_prop.SigmaFission[26] + Mip.el_prop.SigmaCapture[26]) * Mip.NuclearDensity;
                KSI_SIGMA_s += Mip.el_prop.KsiElasticScattering[25] * Mip.el_prop.SigmaElasticScattering[25] * Mip.NuclearDensity;
            }
            double ngt = T * (1 + 0.46 * SIGMA_a / KSI_SIGMA_s);//температура нейтроннго газа
            //поправки тепловых констант с учетом температуры нейтроннго газа
            foreach (var item in this)
                item.InitTermalProperties(ngt);

            return ngt;
        }

        internal void SetSelfScreening()
        {
            foreach (var item in this)
                item.CalcSigma0(this);
        }
    }
    ...
    {
        private MSNCalculation msnCalc;
        public MacroIsotopePropertiesList elements;
        ...
        public double[] SigmaElasticZamedlenie;
        public double[] MuElasticScattering;
        public double[] KsiElasticScattering;

        public double[,] MacroMatrix;

        public double N_Gas_temperature;

        public MacroParams(MSNCalculation msn)
        {
            msnCalc = msn;
            if (msn == null) return;
            SigmaTotal = new double[msnCalc.GroupCount];
            SigmaFission = new double[msnCalc.GroupCount];
            NuFission = new double[msnCalc.GroupCount];
            SigmaCapture = new double[msnCalc.GroupCount];
            SigmaElasticScattering = new double[msnCalc.GroupCount];
            SigmaInelasticScattering = new double[msnCalc.GroupCount];
            MuElasticScattering = new double[msnCalc.GroupCount];
            KsiElasticScattering = new double[msnCalc.GroupCount];
            MacroMatrix = new double[msnCalc.GroupCount, msnCalc.GroupCount];
        }

        internal void Init(MacroIsotopePropertiesList ell)
        {
            elements = ell;
            //передварительная инициализация данных по микросеченим и другим параметрам с учетом температуры и других настроек
            //тепловые сечения элеменов с учетом температуры нейтронного газа
            N_Gas_temperature = elements.InitTermalProperties(msnCalc.Temperature);
            if (msnCalc.isSelfScreening)
                elements.SetSelfScreening();
            //теперь можно рассчитывать макропараметры среды
            foreach (MacroIsotopeProperties el in elements)
                for (int i = 0; i < msnCalc.GroupCount; i++)
                {
                    SigmaTotal[i] += el.NuclearDensity * 1;//el.SigmaTotal[i];
                }
        }
    }

    public class MacroIsotopeProperties
    {
        private MSNCalculation msnCalc;
        public double baseNuclearDensity;
        private IsotopeProperties origin_el;
        public IsotopeProperties el_prop;
        public double NuclearDensity;
        public double[] sigma0;

        public MacroIsotopeProperties(IsotopeProperties or, double nd, MSNCalculation msn)
        {
            origin_el = or;
            baseNuclearDensity = nd;
            msnCalc = msn;
            el_prop = new IsotopeProperties(origin_el);
            //пересчитываем ядерную плотность с учетом температуры
            NuclearDensity = el_prop.leProperties.GetNuclearDensity(baseNuclearDensity, msnCalc.Temperature);
        }

        internal void InitTermalProperties(double ngt)
        {
            //рассчитываем тепловые константы с учетом температуры (см. раздел 2.4 методички)
            double a = Math.Sqrt(293 / ngt) * Math.Sqrt(Math.PI / 4);
            el_prop.SigmaFission[26] = el_prop.SigmaFission[26] * a;
            el_prop.SigmaCapture[26] = el_prop.SigmaCapture[26] * a;
            el_prop.SigmaTotal[26] = el_prop.SigmaFission[26] + el_prop.SigmaCapture[26] + el_prop.SigmaElasticScattering[26] + el_prop.SigmaInelasticScattering[26];
        }

        internal void CalcSigma0(MacroIsotopePropertiesList mList)
        {
            sigma0 = new double[26];
            for (int i = 0; i < 26; i++)
            {
                foreach (var item in mList)
                    if (item.el_prop.Name != this.el_prop.Name)
                    {
                        sigma0[i] += item.el_prop.SigmaTotal[i] * item.NuclearDensity;
                    }
                sigma0[i] = sigma0[i] / this.NuclearDensity;
            }
        }
    }
    ...
    public class IsotopeProperties : IXmlSaveLoad
    {
        ...
        public IsotopeProperties(IsotopeProperties el)
        {
            Name = el.Name;
            isSourceble = el.isSourceble;
            Mass = el.Mass;
            leProperties = el.leProperties;
            rbcProperties = el.rbcProperties;
            ...
            for (int i = 0; i < 26; i++)
            {
                SigmaTotal[i] = el.SigmaTotal[i]; 
                SigmaFission[i] = el.SigmaFission[i];
                NuFission[i] = el.NuFission[i];
                SigmaCapture[i] = el.SigmaCapture[i];
                SigmaElasticScattering[i] = el.SigmaElasticScattering[i];
                SigmaInelasticScattering[i] = el.SigmaInelasticScattering[i];
                SigmaElasticZamedlenie[i] = el.SigmaElasticZamedlenie[i];
                MuElasticScattering[i] = el.MuElasticScattering[i];
                KsiElasticScattering[i] = el.KsiElasticScattering[i];
                for (int j = 0; j < 26; j++)
                {
                    MatrixElasticScattering[i, j] = el.MatrixElasticScattering[i, j];
                    MatrixInelasticScattering[i, j] = el.MatrixInelasticScattering[i, j];
                }
            }
        }
        ...
        public void LoadFromXml(XmlNode node)
        {
            ...
            if (ch != null)
            {
                ...
                xmlHelp.GetDataLineArray(ch, ref SigmaElasticScattering, 26, "SigmaElasticScattering");
                xmlHelp.GetDataLineArray(ch, ref SigmaInelasticScattering, 26, "SigmaInelasticScattering");
                xmlHelp.GetDataLineArray(ch, ref SigmaElasticZamedlenie, 26, "SigmaElasticZamedlenie");
                xmlHelp.GetDataLineArray(ch, ref MuElasticScattering, 26, "MuElasticScattering");
                xmlHelp.GetDataLineArray(ch, ref KsiElasticScattering, 26, "KsiElasticScattering");
            }
